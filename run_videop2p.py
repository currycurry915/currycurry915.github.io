...
from tuneavideo.pipelines.pipeline_tuneavideo_without_flow import TuneAVideoPipeline
...
def main(
    ...
    # print('run_videop2p_refine ln55', prompt)
    # print('same code ln56 len(prompt)', len(prompt))
    output_folder = os.path.join(pretrained_model_path, 'original_result_' + now_str)
    if fast:
        ...
        def __init__(self, prompts: List[str], words: [List[List[str]]], substruct_words=None, start_blend=0.2, th=(.3, .3)):
            ...
            for i, (prompt, words_) in enumerate(zip(prompts, words)):
                if type(words_) is str:
                    words_ = [words_]
                for word in words_:
                    ind = ptp_utils_original.get_word_inds(prompt, word, tokenizer)
    ...
    class AttentionControlEdit(AttentionStore, abc.ABC):
        ...
        def __init__(self, prompts, num_steps: int,
            ...
            self.cross_replace_alpha = ptp_utils_original.get_time_words_attention_alpha(prompts, num_steps, cross_replace_steps, tokenizer).to(device)
    ...
    def get_equalizer(text: str, word_select: Union[int, Tuple[int, ...]], values: Union[List[float],
        ...
        for word, val in zip(word_select, values):
            inds = ptp_utils_original.get_word_inds(text, word, tokenizer)
            equalizer[:, inds] = val
        return equalizer
    ...
    class NullInversion:
        ...
        def invert(self, image_path: str, prompt: str, offsets=(0,0,0,0), num_inner_steps=10, early_stop_epsilon=1e-5, verbose=False):
            ...
            ptp_utils_original.register_attention_control(self.model, None)
    ...
    ptp_utils_original.register_attention_control(ldm_stable, controller)
