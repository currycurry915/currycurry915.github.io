...
import ptp_utils_original

from torchvision import transforms

from copy_code_jih import aggregate_attention
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                ptp_utils_original.register_attention_control(self, controller)
                noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=text_embeddings).sample.to(dtype=latents_dtype)
                attention_maps = aggregate_attention(controller, 16, ["up", "down"], True, select = 0, prompts =prompt)
                
                # print("attention_maps.shape",attention_maps.shape)
                #attention_maps.shape=torch.Size([8, 16, 16, 77])

                # if i==49:
                #     for j in range(6):
                #         attention_maps_1 = attention_maps[:, :, :,j].detach()
                #         resize = transforms.Resize(64)
                #         attention_maps_1 = resize(attention_maps_1)
                #     # only for save attn
                #         for k in range (8):
                #             image = attention_maps_1[0]
                #             image = 255 * image / image.max()
                #             image = image.unsqueeze(-1).expand(*image.shape, 3)
                #             image = image.numpy().astype(np.uint8)
                #             image_64 = Image.fromarray(image).convert('L').resize((64, 64))
                #             convert_tensor = transforms.ToTensor()
                #             image_to_tensor = convert_tensor(image_64)
                #             image = Image.fromarray(image).resize((256, 256))
                #             image.save('/database/WACV_jih/outputs/cloud_tower/attn1/'+str(j)+'_'+str(k).zfill(3)+'.png', 'png')

                            # image_2 = attention_maps_2[k]
                            # image_2 = 255 * image_2 / image_2.max()
                            # image_2 = image_2.unsqueeze(-1).expand(*image_2.shape, 3)
                            # image_2 = image_2.numpy().astype(np.uint8)
                            # image_64_2 = Image.fromarray(image_2).convert('L').resize((64, 64))
                            # convert_tensor = transforms.ToTensor()
                            # image_to_tensor = convert_tensor(image_64_2)
                            # image_2 = Image.fromarray(image_2).resize((256, 256))
                            # image_2.save('/database/jih/airplane/attn_2/'+str(k).zfill(3)+'.png', 'png')
